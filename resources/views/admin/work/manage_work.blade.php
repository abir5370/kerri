                            ...
                            <table id="dataTableExample" class="table">
                                ...
                                @foreach($works as $key=>$work)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{route('status.work',$work->id)}}" class="btn btn-{{$work->status == 1?'success':'secondary'}}">{{$work->status == 1?'Active':'Deactive'}}</a>
                                        </td>
                                        <td class="btn-group">
                                            <a href="{{route('edit.work',$work->id)}}" class="btn btn-sm btn-primary mx-1">Edit</a>
                                            <form action="{{route('delete.work')}}" method="post">
                                                @csrf
